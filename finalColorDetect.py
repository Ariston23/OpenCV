...
def getColorDisability():
    ...
    while True:
        colorDisability = input("Please Enter The Number For The Kind Of Colorblindedness You Would Like Cured \n   1. Deuteranopia \n   2. Protanopia \n   3. Tritanopia \n")
        if colorDisability == "1" or colorDisability == "2" or colorDisability == "3":
            return colorDisability
        else:
            print("Invalid Entry\n")
...
def main():
    ...
    colorDisability = getColorDisability()

    blackFiltered = cv2.imread(args["image"])
    blueFiltered = cv2.imread(args["image"])
    greenFiltered = cv2.imread(args["image"])
    redFiltered = cv2.imread(args["image"])
    brownFiltered = cv2.imread(args["image"])
    orangeFiltered = cv2.imread(args["image"])
    violetFiltered = cv2.imread(args["image"])
    whiteFiltered = cv2.imread(args["image"])
    yellowFiltered = cv2.imread(args["image"])
    ...
    if colorDisability == "1":
    ...
    elif colorDisability == "2":
        for color in frequent_data:
            if color[0] == "Shades of Red":
                offset = 100
    ...
    elif colorDisability == "3":
        for color in frequent_data:
            if color[0] == "Shades of Brown":
                offset = 90
        ...
        cv2.imshow("Blue Filter", brownFiltered)
        cv2.imshow("Original Image", image)
